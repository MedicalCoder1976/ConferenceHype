...
export function PreparedNarrativeBroadcast() {
  ...
  return <div className="border-2 border-broadcast/30 bg-white p-5 shadow-panel">
    ...
    <p className="mt-2 text-sm font-semibold leading-6 text-ink/65">Paste Claude&apos;s complete ConferenceHype JSON package. Both the full prepared-broadcast schema and the five-news Meeting Watch schema are accepted. Ask Claude to name the article title and authors only in the opening, then begin the evidence immediately. Do not repeat show labels, host descriptors, or phrases such as &quot;A new ASCO Educational Book review.&quot; Keep each visible card concise, include the disclaimer once, and end with a brief invitation to like, subscribe, and recommend the next article. We enforce these rules without changing station or journal schedules.</p>
    <textarea value={raw} onChange={(event) => { setRaw(event.target.value); setPreview(null); }} rows={14} placeholder='Paste JSON beginning with "conferencehype_prepared_broadcast_v1" or "conferencehype_meeting_watch_five_news_v1".' className="mt-4 w-full border border-ink/20 px-3 py-3 font-mono text-xs text-ink" />
