...
export function PreparedNarrativeBroadcast() {
  ...
  const publish = () => startTransition(async () => {
    try { const response = await fetch("/api/admin/meeting-watch/prepared/publish", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ raw }) }); const payload = await response.json(); if (!response.ok || !payload.ok) throw new Error(payload.error ?? "Could not start this broadcast."); setMessage(payload.alreadyExists ? "This exact package already exists; no duplicate was dispatched." : `Broadcast render started: ${payload.cardCount} cards, ${Math.round(payload.durationSeconds / 60)} estimated minutes, ${payload.speakerTurnCount} speaker turns.`); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not start this broadcast."); }
  });
  return <div className="border-2 border-broadcast/30 bg-white p-5 shadow-panel">
    ...
    <p className="mt-2 text-sm font-semibold leading-6 text-ink/65">Paste Claude&apos;s complete ConferenceHype JSON package. We remove harmless preamble text, validate every required field, preserve HOST 1 and HOST 2 as separate voices, calculate the real program length, and render without changing the station or journal schedules.</p>
