...
export function PreparedNarrativeBroadcast() {
  ...
  return <div className="border-2 border-broadcast/30 bg-white p-5 shadow-panel">
    ...
    <textarea value={raw} onChange={(event) => { setRaw(event.target.value); setPreview(null); setTitle(""); setThumbnailStatement(""); }} rows={14} placeholder='Paste Claude JSON beginning with { "schema_version": "conferencehype_meeting_watch_five_news_v1" ...' className="mt-4 w-full border border-ink/20 px-3 py-3 font-mono text-xs text-ink" />
