...
export function BreakingPaperDesk() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); setMessage("Reading the paper and preparing a evidence-based broadcast…"); setCreated(null);
    try {
      const response = await fetch("/api/admin/breaking-paper", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sourceUrl, placement, socialReaction: socialReaction.trim() || undefined }) });
  ...
  return <section className="grid gap-5 border border-ink/15 bg-white p-5 shadow-panel">
    <div className="flex items-start gap-3"><BookOpenText className="mt-1 h-6 w-6 text-broadcast" /><div>
      <div className="text-xs font-black uppercase text-broadcast">Clinical Evidence Brief</div><h2 className="text-2xl font-black text-ink">Breaking Paper broadcast</h2>
      <p className="mt-2 max-w-3xl text-sm font-semibold text-ink/65">Paste an important PubMed or publisher paper link. ConferenceHype will use the page&apos;s title and abstract to prepare a focused, evidence-based broadcast without a paid AI call.</p>
    </div></div>
    <form className="grid gap-3" onSubmit={submit}>
      ...
      <label className="grid gap-1 text-xs font-black uppercase text-ink/70">X/Twitter reaction (optional)<textarea value={socialReaction} onChange={(event) => setSocialReaction(event.target.value)} maxLength={2000} rows={3} placeholder="Paste a tweet's text here to include it as an attributed reaction beat in the broadcast." className="border border-ink/20 px-3 py-2 text-sm font-semibold normal-case text-ink" /><span className="font-semibold normal-case text-ink/50">Pasted by hand — no X API call. Read aloud as a clearly attributed &quot;on X&quot; beat, never mixed with the paper&apos;s own findings.</span></label>
