...
begin
  ...
  if extract(isodow from candidate.schedule_date) not between 1 and 5 then
    raise exception 'Regular journal activation is Monday-Friday only.';
  end if;
  ...
  on conflict (schedule_id, position) do update set
    specialty=excluded.specialty, journal_id=excluded.journal_id,
    journal_name=excluded.journal_name, program_type=excluded.program_type,
    source_program_id=excluded.source_program_id,
    starts_at_offset_minutes=excluded.starts_at_offset_minutes,
    status=excluded.status, youtube_video_id=excluded.youtube_video_id,
  ...
  set status='active', activated_at=now(),
    verification_summary=verification_summary || jsonb_build_object(
      'mode','one_daily_top_journal_release',
      'youtube_release_times_eastern',jsonb_build_array('06:15'),
      'new_youtube_videos',1,'replay_positions',5,
      'prepared_previous_evening',true,'minimum_publication_delay_days',1,
      'top_journal_cadence_days',14,'unused_cards_preserved',true),
    updated_at=now()
