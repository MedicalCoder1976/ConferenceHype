...
begin
  ...
  select count(*), count(distinct youtube_video_id) into originals, distinct_videos
  from public.station_programs
  where schedule_id = p_schedule_id and position in (0, 1) and program_type = 'new'
    and status = 'verified' and youtube_video_id is not null;
  if originals <> 2 or distinct_videos <> 2 then raise exception 'Both weekday programs must be verified with distinct YouTube videos before activation.'; end if;
  if (select count(*) from public.station_programs where schedule_id = p_schedule_id and program_type = 'new') <> 2 then
    raise exception 'Regular programming permits exactly two new journal releases per day.';
  end if;
  ...
  select p_schedule_id, repeated.position, source.specialty, source.journal_id,
    source.journal_name, 'journal_replay', source.id, repeated.position * 30, 30,
  ...
  join public.station_programs source on source.schedule_id = p_schedule_id and source.position = (repeated.position % 2)
  ...
  if (select count(*) from public.station_programs where schedule_id = p_schedule_id and status = 'verified') <> 6 then
    raise exception 'Daily station schedule must contain two releases and four verified replay positions.';
  end if;
  ...
  set status='active', activated_at=now(),
    verification_summary=verification_summary || jsonb_build_object(
      'mode', 'two_daily_journal_releases',
      'youtube_release_times_eastern', jsonb_build_array('07:15', '17:10'),
      'new_youtube_videos', 2, 'replay_positions', 4,
