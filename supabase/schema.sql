...
create table public.oncology_journals (
  ...
  last_issue_key text,
  specialty text,
  regional_only boolean not null default false,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table public.journal_series_memberships (
...
create table public.regional_journal_programs (
  id uuid primary key default gen_random_uuid(),
  series_id uuid not null references public.journal_series(id) on delete restrict,
  release_date date not null,
  starts_at timestamptz not null,
  status text not null default 'planned' check (status in ('planned', 'shadow_verified', 'rendering', 'verified', 'failed', 'cancelled')),
...
create table public.journal_article_sync_state (
...
create index if not exists journal_articles_card_segment_idx
  on public.journal_articles (card_segment_id) where card_segment_id is not null;
create index if not exists journal_articles_doi_idx
  on public.journal_articles (journal_id, lower(doi)) where doi is not null and doi <> '';
