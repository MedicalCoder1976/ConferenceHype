...
Provide 5-10 distinct, source-grounded abstracts. Each narration must be 30-65 words and must be the exact spoken copy. Give every abstract a supporting primary-source URL. One primary-source document may support multiple distinct abstracts or studies; when reusing its URL, provide a distinct abstract_number or study_name for every item. Include exact study names and numbers only when supported. Move naturally from one abstract to the next without phrases that sound mechanically attached. Do not write host names, production directions, slide directions, music cues, markdown, or repeated show introductions.
