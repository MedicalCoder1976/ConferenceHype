...
export function buildMeetingWatchMetadata({
  ...
  const fiveThingsDescription = [
    ...
    ...chapters,
    "",
    ...(fiveThingSources.length ? ["Primary sources:"] : []),
    ...fiveThingItems.flatMap(({ segment }, index) => segment.citations[0]?.url ? [`${index + 1}. ${segment.title}: ${segment.citations[0].url}`] : []),
    "",
    "Subscribe for the next specialty briefing from ConferenceHype.",
    "",
  ...
  return {
    ...
    thumbnailHeadline: isPreparedFiveThings ? title : isPreparedStory ? preparedThumbnail ?? (storyOrigin ? "From " + storyOrigin + "?" : packaging.thumbnailHook) : meetingTopic,
    thumbnailHook: isPreparedFiveThings ? title : isPreparedStory ? preparedThumbnail ?? (storyOrigin ? "From " + storyOrigin + "?" : packaging.thumbnailHook) : meetingTopic,
