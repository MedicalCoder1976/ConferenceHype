...
export function buildSocialVoiceCompetitionSegment(
  ...
  const board = topThree
    ...
    .join("\n");
  const routing =
    "Tag #ConferenceHype on X or Instagram to nominate the next monitored voice, official schedule item, article, or media moment.";
  ...
  return {
    ...
    script: `Social voice scoreboard check. Every hour, ConferenceHype ranks the voices lighting up the medical-conference conversation on X. Top voices are added to the monitored callout list for source-attributed broadcast commentary.\n\n${board || "The board is still warming up. More source-attributed conference signal is needed before we can crown a leader."}\n\n${routing}`,
