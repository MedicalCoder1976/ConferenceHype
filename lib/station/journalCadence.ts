...
export function orderedCadenceJournals(targetDate: string, journals: OncologyJournal[]) {
  ...
  const preferred = names.map((name) => byName.get(name.toLowerCase())).filter((journal): journal is OncologyJournal => Boolean(journal?.enabled && !journal.regionalOnly));
  const preferredIds = new Set(preferred.map((journal) => journal.id));
  const sidelined = journals
    .filter((journal) => journal.enabled && !journal.regionalOnly && !preferredIds.has(journal.id))
