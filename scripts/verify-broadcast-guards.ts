...
assert.match(renderHourSource, /\(process\.env\.STATION_PROGRAM_ID \|\| process\.env\.REGIONAL_PROGRAM_ID \|\| isMeetingWatchMode\) \? process\.env\.YOUTUBE_PUBLISH_AT/);
...
assert.match(renderHourSource, /\["JOURNAL CLUB", \.\.\.\(actualMetadata\?\.relevantSpecialties \?\? \[\]\)\]\.join\(" \| "\)/);
assert.match(renderHourSource, /\["JOURNAL CLUB", \.\.\.weekendSpecialtiesForRender\]\.join\(" \| "\)/);
assert.match(renderHourSource, /: "CLINICAL EVIDENCE BRIEF"/);
...
assert.match(renderHourSource, /if \(isJournalMode \|\| isRegionalMode \|\| isMeetingWatchMode \|\| process\.env\.STATION_PROGRAM_ID\) throw error/);
