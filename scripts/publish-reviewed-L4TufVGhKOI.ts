...
async function setLocalizedPackaging(metadata: Metadata, videoId: string, accessToken: string) {
  const current = await fetch("https://www.googleapis.com/youtube/v3/videos?part=snippet&id="+videoId, {headers:{Authorization:"Bearer "+accessToken}});
  if (!current.ok) throw new Error("Cannot read uploaded metadata");
  const item = (await current.json()).items[0];
  const response = await fetch("https://www.googleapis.com/youtube/v3/videos?part=snippet", {
    method:"PUT", headers:{Authorization:"Bearer "+accessToken,"Content-Type":"application/json"},
    body:JSON.stringify({id:videoId,snippet:{title:metadata.title,description:metadata.description,categoryId:item.snippet.categoryId,tags:["ConferenceHype","Novo Nordisk","CagriSema",metadata.language_name],defaultLanguage:metadata.language,defaultAudioLanguage:metadata.language}})
  });
  if(!response.ok) throw new Error("Language metadata update failed: "+await response.text());
  const thumbnail = await readFile(metadata.thumbnail_path);
  const result = await fetch("https://www.googleapis.com/upload/youtube/v3/thumbnails/set?videoId="+videoId, {
    method:"POST",headers:{Authorization:"Bearer "+accessToken,"Content-Type":"image/png"},body:thumbnail
  });
  if(!result.ok) throw new Error("Thumbnail upload failed: "+await result.text());
}
...
async function main() {
  ...
  if (existingVideoId) {
    ...
    try {
      ...
      try {
        ...
        await setLocalizedPackaging(metadata, existingVideoId, accessToken);
  ...
  const uploaded = await uploadVideoToYoutube({
    ...
    tags: ["ConferenceHype", "Novo Nordisk", "CagriSema", "REIMAGINE 5", "REDEFINE 9", metadata.language_name],
    categoryId: "28",
  ...
  await setLocalizedPackaging(metadata, uploaded.id, accessToken);
