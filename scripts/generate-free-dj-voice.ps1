...
$wavPath = Join-Path $recordingsDir "tumorcrusher-fenrir-rebecca-adam-day1-run-v1.wav"
$mp3Path = Join-Path $recordingsDir "tumorcrusher-fenrir-rebecca-adam-day1-run-v1.mp3"
$trimmedPath = Join-Path $recordingsDir "tumorcrusher-fenrir-rebecca-adam-day1-run-v1.trimmed.mp3"
$voices = @("am_fenrir", "af_bella", "am_adam")
...
foreach ($voiceName in $voices) {
  ...
  if (Test-Path -LiteralPath $voiceWav) {
    $voiceFilter = if ($voiceName -eq "am_adam") { $adamVoiceFilter } elseif ($voiceName -eq "af_bella") { $rebeccaVoiceFilter } else { $standardVoiceFilter }
...
Remove-Item -LiteralPath $wavPath -Force -ErrorAction SilentlyContinue
Write-Host "Open-source Kokoro Fenrir schedule / Rebecca reporter / Adam social recording generated: $mp3Path"
