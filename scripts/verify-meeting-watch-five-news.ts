...
assert.equal(fullNarrativeSegments.filter((segment) => segment.riskFlags.some((flag) => flag === "prepared_transition:16")).length, 11);
...
assert.ok(fullNarrativeMusic.slice(0, -1).every((slot) => slot.durationSeconds === 16));
...
assert.match(renderSource, /assertNarrationWindowsAudible/);
...
assert.match(meetingWatchDeskSource, /JSON\.stringify\(\{ raw, title, thumbnailStatement \}\)/);
