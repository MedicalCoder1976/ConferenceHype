...
- Every narrator uses the shared spoken-only pharma pronunciation list in `PHARMA_SPOKEN_PRONUNCIATIONS`: AbbVie (`Abb-vee`), Amgen (`Am-jen`), AstraZeneca (`As-truh-ZEN-eh-kuh`), BioNTech (`Bye-ON-tek`), Boehringer Ingelheim (`Bear-ing-er Ing-el-hime`), Bristol Myers Squibb, Daiichi Sankyo (`Dye-ee-chee Sank-yoh`), Eli Lilly, Genentech (`Jen-en-tek`), Gilead (`Gil-ee-ad`), GlaxoSmithKline, GSK (`G S K`), Merck (`Murk`), Novartis (`no-VAR-tis`), Novo Nordisk, Pfizer (`Fye-zer`), Regeneron (`Reh-jen-er-on`), Roche (`Rohsh`), Sanofi (`Sah-no-fee`), Takeda (`Tah-keh-dah`), and Taiho (`Tie-ho`). These substitutions affect synthesized audio only; visible company names remain unchanged. The same pre-synthesis cleanup removes punctuation inside `objective response rate` so the phrase is spoken continuously and collapses adjacent `novel-novel` or `novel novel` to one `novel`.
...
- Topic handoffs must remain audibly filled and concise. Journal, weekend-roundup, and regional-journal transitions use 20-second speech-free music gaps. Complete-narrative Meeting Watch transitions use 16 seconds, a 20 percent reduction that keeps the story moving. The shared renderer loops and trims the approved gap clip to the exact scheduled transition window so a longer slot cannot end with dead silence after the clip fades.
...
- New Claude or Grok packages use schema `conferencehype_meeting_watch_full_narrative_v3`. The model supplies the exact beginning-to-end spoken text: a focused 30-120-word hook that identifies the meeting and its year or dates, names at least two source-supported pharma companies, and previews the subjects; 5-10 ordered, distinct abstracts with 30-65 exact spoken words and a supporting primary-source URL; a 15-35-word disclaimer; and a 20-50-word closing. A single primary-source document may support multiple abstracts when each has a distinct abstract number or study name. ConferenceHype preserves that narration order verbatim, inserts 16 seconds of speech-free music between the hook, abstracts, and closing, and rejects packages whose calculated narration plus music exceeds 10 minutes. Claude/Grok must keep `objective response rate` unbroken and must not write `novel-novel` or consecutive repetitions of `novel`. The admin uses one `Develop and publish YouTube video` action; validation, title/thumbnail normalization, render, upload, and delivery tracking remain server-side. The legacy `conferencehype_meeting_watch_five_news_v1` and `conferencehype_meeting_watch_story_v2` schemas remain readable for existing packages.
...
- The Meeting Watch admin extracts the editable YouTube headline directly from `meeting.eye_catching_topic` as soon as pasted JSON can be parsed, even if unrelated narration validation still needs correction. The operator-edited headline is passed through unchanged at publish time, subject to the 150-character and prohibited-generic-label guards.
