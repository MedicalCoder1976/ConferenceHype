...
- New Claude or Grok packages use schema `conferencehype_meeting_watch_full_narrative_v3`. The model supplies the exact beginning-to-end spoken text: a focused 30-120-word hook that identifies the meeting and its year or dates, names at least two source-supported pharma companies, and previews the subjects; 5-10 ordered, distinct abstracts with 30-65 exact spoken words and a supporting primary-source URL; a 15-35-word disclaimer; and a 20-50-word closing. A single primary-source document may support multiple abstracts when each has a distinct abstract number or study name. ConferenceHype preserves that narration order verbatim, inserts 20 seconds of speech-free music between the hook, abstracts, and closing, and rejects packages whose calculated narration plus music exceeds 10 minutes. The admin uses one `Develop and publish YouTube video` action; validation, title/thumbnail normalization, render, upload, and delivery tracking remain server-side. The legacy `conferencehype_meeting_watch_five_news_v1` and `conferencehype_meeting_watch_story_v2` schemas remain readable for existing packages.
